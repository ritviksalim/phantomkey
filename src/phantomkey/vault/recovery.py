...
"""Modular recovery system for vault lockout scenarios.

Provides a RecoveryStrategy interface with two implementations:
- PhraseRecovery: BIP-39 style 12-word mnemonic
- KeyRecovery: Opaque PK-prefixed recovery key

To add a new strategy, subclass RecoveryStrategy and implement
generate(), derive_kek(), and display_name().
"""

import secrets
from abc import ABC, abstractmethod

from phantomkey.vault.crypto import derive_kek as _derive_kek

# BIP-39 inspired word list (2048 words). Using a curated subset for simplicity.
...
_WORDLIST = [
    ...
    "angry", "animal", "answer", "anxiety", "any", "apart", "apology", "appear",
    "apple", "approve", "arctic", "area", "arena", "argue", "armor", "army",
    "arrive", "arrow", "art", "asset", "assist", "assume", "asthma", "atom",
    "attack", "attend", "auction", "audit", "august", "aunt", "author", "auto",
    "avocado", "avoid", "awake", "aware", "autumn", "average", "axis", "baby",
    "bachelor", "bacon", "badge", "bag", "balance", "balcony", "ball", "bamboo",
    "banana", "banner", "bar", "barrel", "base", "basket", "battle", "beach",
    "bean", "beauty", "become", "beef", "before", "begin", "behave", "believe",
    "bench", "benefit", "best", "betray", "better", "beyond", "bicycle", "bird",
    "birth", "bitter", "blade", "blame", "blanket", "blast", "bleak", "bless",
    ...
    "burst", "bus", "busy", "butter", "buyer", "cabin", "cable", "cactus",
    "cage", "cake", "call", "calm", "camera", "camp", "canal", "cancel",
    "candle", "candy", "cannon", "canvas", "canyon", "capable", "capital", "captain",
    "carbon", "card", "cargo", "carpet", "carry", "cart", "case", "castle",
    "casual", "cat", "catalog", "catch", "cattle", "cause", "caution", "cave",
    ...
    "choice", "chunk", "circle", "citizen", "city", "civil", "claim", "clap",
    ...
    "finish", "fire", "firm", "fiscal", "fish", "fit", "fitness", "flag",
    ...
    "glad", "glance", "glare", "glass", "gleam", "globe", "gloom", "glory",
    ...
    "ignore", "image", "imitate", "immense", "immune", "impact", "impose", "improve",
    "impulse", "inch", "include", "income", "increase", "index", "indicate", "indoor",
    "industry", "infant", "inflict", "inform", "initial", "inject", "inmate", "inner",
    ...
    "kangaroo", "keen", "keep", "kernel", "key", "kick", "kid", "kidney",
    ...
    "leaf", "learn", "leave", "lecture", "left", "leg", "legend", "leisure",
    ...
    "morning", "mouse", "move", "movie", "much", "muffin", "mule", "multiply",
    ...
    "night", "noble", "noise", "nominee", "normal", "north", "nose", "notable",
    ...
    "orbit", "orchard", "order", "organ", "orient", "original", "orphan", "ostrich",
    ...
    "twist", "type", "typical", "ugly", "umbrella", "unable", "unaware", "uncle",
...
class RecoveryStrategy(ABC):
    """Abstract base for recovery credential generation and derivation."""

    @abstractmethod
    def generate(self) -> str:
        """Generate a new recovery credential (phrase, key, etc.)."""
        ...

    @abstractmethod
    def derive_kek(self, recovery_credential: str, salt: bytes) -> bytes:
        """Derive a KEK from the recovery credential."""
        ...

    @abstractmethod
    def display_name(self) -> str:
        """Human-readable name for this strategy (e.g., 'recovery phrase')."""
        ...

    @abstractmethod
    def strategy_id(self) -> str:
        """Short identifier stored in vault file (e.g., 'phrase', 'key')."""
        ...


class PhraseRecovery(RecoveryStrategy):
    """BIP-39 style 12-word recovery phrase."""

    def generate(self) -> str:
        words = [secrets.choice(_WORDLIST) for _ in range(12)]
        return " ".join(words)

    def derive_kek(self, recovery_credential: str, salt: bytes) -> bytes:
        passphrase = recovery_credential.strip().lower().encode("utf-8")
...
class KeyRecovery(RecoveryStrategy):
    """Opaque PK-prefixed recovery key."""

    def generate(self) -> str:
        # Generate 24 random bytes, encode as base58-like alphanumeric
        ...
        while num > 0:
            ...
            num //= 62
        return "PK-" + "".join(reversed(result))

    def derive_kek(self, recovery_credential: str, salt: bytes) -> bytes:
        passphrase = recovery_credential.strip().encode("utf-8")
        return _derive_kek(passphrase, salt)
...
def pick_challenge_words(phrase: str, count: int = 3) -> tuple[list[int], list[str]]:
    """Pick random words from a phrase for confirmation challenge.

    Returns (indices, words) where indices are 0-based positions.
    """
    words = phrase.split()
    indices = sorted(secrets.SystemRandom().sample(range(len(words)), count))
